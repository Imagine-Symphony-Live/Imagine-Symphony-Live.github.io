...
export default class OverWorldState extends State {
  private viewport: FancyViewport;

  async createContainer(app: Application): Promise<Container> {
    this.viewport = new FancyViewport({
    ...
    this.viewport.viewPort.on("clicked", (d: ClickEventData) => {
      const {x, y} = d.screen;
    ...
    this.viewport.addChild(bioContainer);
    ...
    musicianBios.forEach((b) => {
      b.on("focused", () => {
        ...
        setTimeout(() => {
          ...
          this.viewport.transitionCenter(panx, pany, 500);
        }, 100);
      });
      b.on("activated", () => {
